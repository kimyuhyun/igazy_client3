...
import { interpolateEyeAngle, CORRECTION_FACTOR } from "../utils/EyeAngleCalculator";
import { calcEyeAngle } from "../utils/XAxisCalibrator";
...
export default function PDReport() {
    ...
    const calculatePD = (points, axis, side) => {
        ...
        // ========================================
        // 방법 2: 논문 공식 적용 (VOG 보정)
        // 실제각도 = 0.978 × VOG각도 - 0.549
        // ========================================
        // const correctedMM = differenceMM;
        // const tmpDegrees = interpolateEyeAngle(correctedMM, angle);
        // const degrees = 0.978 * tmpDegrees - 0.549;
        // const radians = degrees * (Math.PI / 180);
        // const pdValue = Math.tan(radians) * 100;

        // ========================================
        // 방법 3: cos 보정 (CamAngle → 30.4° 기준 정규화)
        // ========================================
        // const degrees = calcEyeAngle(differenceMM, angle);
    ...
    const calculatePD_5th_only = (points, axis, side) => {
        if (!points || points.length < 2 || points[0] == null || points[1] == null) {
            return "";
        }
        ...
        // const correctedMM = differenceMM;
        // const degrees = interpolateEyeAngle(correctedMM, angle);
        // const correctedDegrees = 0.978 * degrees - 0.549;
        // const radians = correctedDegrees * (Math.PI / 180);
        // const pdValue = Math.tan(radians) * 100;

        // ========================================
        // 방법 3: cos 보정 (CamAngle → 0° 기준 정규화)
        // ========================================
        // const camAngleRad = (angle * Math.PI) / 180;
        // const calibAngleRad = (CALIB_CAM_ANGLE * Math.PI) / 180;
        // const correctedMM = (differenceMM * Math.cos(camAngleRad)) / Math.cos(calibAngleRad);
        // const degrees = interpolateEyeAngle(correctedMM, angle);
        // const radians = degrees * (Math.PI / 180);
        // const pdValue = Math.tan(radians) * 100;
        ...
        if (axis === "x") {
            if (side === "OD") {
                direction = differenceMM > 0 ? "EXO" : "ESO";
            } else {
                direction = differenceMM > 0 ? "EXO" : "ESO";
            }
        } else {
            direction = differenceMM > 0 ? "HYPO" : "HYPER";
        }
        return `${degrees.toFixed(1)}° / ${pdValue.toFixed(1)} ${direction}`;
    };
